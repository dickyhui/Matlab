function [ output_args ] = NetCDFtoImage_BLD( importFile )
...
try
    ...
    DataType = 'AnlysDgnst';
    MaskValue=9999;
    variable3D={'blthick'};% 三维填色图
    %variableContour3D={'tps'};% 三维等值线
    %variableDir3D={'dir'};% 三维箭头
    %设置全局的图片输出的格式
    Date='BLD';
    OutputDirectory = [OutputDirectory,DataType,'\'];
    SetImageLayout();
    DateTime=name(end-12:end-7);
    fullDir = [OutputDirectory,'BLD','.xml'];
    if(exist(fullDir,'file'))
        ...
        fclose(frid);
        docNode=xmlread(fullDir);%读取XML文件
    ...
    xmlwrite(fullDir,docNode);%保存xml
catch ME
