function [ ] = NetCDFtoImageDirAnlysDgnst(ncid,variables,start,count,levels,times,piclevels,picinterval)
global Time_interval Level_interval DateTime OutputDirectory VectorPicSize Date
%  NetCDF生成箭头
%  ncid：NetCDF的ID
    ...
    case 1  %只有一个参数，画无大小的箭头
        ...
        for level=piclevels
            ...
            while i<levels
                %循环时间
                while j<times
                    name=[Date,'-', variables{1},'-',dateTimeTemp, '-L',num2str(level)];%文件名            
                    %日期加一个月
                    ...
                    %出图的图层路径（包括文件名）
                    fullPath = [OutputDirectory,Date,'\',name, '.png'];
                    %画箭头(矩阵逆时针旋转90°)
    ...
    case 2  %有两个参数，画带大小的箭头
        ...
        for level=piclevels
            ...
            while i<levels
                %循环时间
                while j<times
                    name=[Date,'-', variables{1},variables{2},'-',dateTimeTemp, '-L',num2str(level)];%文件名            
                    %日期加一个月
                    dateTimeTemp=str2double(dateTimeTemp)+1;
                    if(fix(dateTimeTemp/100)>0 && rem(dateTimeTemp,100)==13)%12进1
                        dateTimeTemp=[num2str(fix(dateTimeTemp/100)+1),'01'];
